"""Write a program that allows you to search for a chemical element
by its symbol and get its full name and atomic number.
The program uses a predefined dictionary of elements.
To help you out, you can start by placing this dictionary on top of the script:"""
...
while True:
    ...
    element_user = input('Enter the chemical symbol (i.e. H, He, C ): ')
    if element_user in elements:
        info = elements[element_user]
        print(f'{info["name"]}: Atomic Number {info["atomic_number"]}')
        break

    else:
        print(f'Element "{element_user}" is not valid.')
        print('-------- Try again ---------')
